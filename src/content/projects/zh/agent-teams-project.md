...
使用者先上传一份 PDF 或 DOCX 合同，等待本地文本解析和字段提取，再检查合同双方、金额、日期等结构化字段。风险扫描完成后，工作流按风险等级进入人工决策节点，最后汇总为 JSON 报告，并通过 SSE 把进度推回工作台。
...
React 工作台通过 FastAPI 进入合同、会话、字段、事件和报告接口。字段提取完成后，LangGraph 风险路由决定是进入逐条人工审核、批量处理还是自动通过；SQLite 持久化合同审核状态，SSE 负责把进行中的事件传回页面。
...
把合同解析和结构化字段提取放在 LangGraph 风险路由之前，让人工先看到合同双方、金额、生效日期等可核验信息，也避免把文件读取和风险决策混成一个不可定位的步骤。

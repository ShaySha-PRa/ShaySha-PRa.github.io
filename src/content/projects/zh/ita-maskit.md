...
Excel 中若把长标识符存成数字，文件格式本身可能先丢失精度，工具无法恢复被截断的末位；这类字段应保持为文本。WSL 中的 Ruff 静态检查仍报告源仓库已有的 15 个格式或未使用项，本页不把它写成静态检查通过；项目也不宣称通过任何认证或构成合规解决方案。下一步是在更多文件边界上补充可重复的 CLI 验收记录，并单独评估 beta 格式路径。
